...
void MediaPlayer::start_timer()
{
    std::time_t tt = std::time(nullptr);
    std::tm* calendar_time = std::localtime(&tt);
    int seconds_left = ((59 - calendar_time->tm_min + *m_timer_start_minute) % 60) * 60 + (59 - calendar_time->tm_sec);
...
//    set_default_config_path("~/");
    ...
    m_timer_restart_movie = Timer(main_queue().io_service(), [this]()
    {
        if(m_media){ m_media->restart(); }
    });

    // schedule playback every full hour
    start_timer();
...
void MediaPlayer::draw()
{
    ...
    if(!m_timer_restart_movie.has_expired())
    {
        ...
        m->set_scale(m_current_scale);

        // position centered
        m->set_position(vec3((gl::window_dimension() - sz) / 2.f , 0.f));

        gl::ScopedMatrixPush pro(gl::PROJECTION_MATRIX), mod(gl::MODEL_VIEW_MATRIX);
