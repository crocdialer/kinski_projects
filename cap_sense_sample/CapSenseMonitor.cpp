...
void CapSenseMonitor::draw()
{
    ...
    for(auto &s : m_sensors)
    {
        for(int i = 0; i < 13; i++)
        {
            ...
            float proxi_val = crocore::clamp(*m_cap_sense_proxi_multiplier * s->proximity_values()[12 - i] / 100.f,
                                             0.f, 1.f);
...
void CapSenseMonitor::update_property(const PropertyConstPtr &theProperty)
{
...
void CapSenseMonitor::send_udp_broadcast()
{
    ...
    for(auto &s : m_sensors)
    {
        ...
        for(int j = 0; j < 3; ++j)
        {
            vals[j] = crocore::clamp(s->proximity_values()[j] * *m_cap_sense_proxi_multiplier / 100.f, 0.f, 1.f);
        }
