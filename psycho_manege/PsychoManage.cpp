...
{
    PsychoManage *app = static_cast<PsychoManage *>(userData);

    // schedule on main queue
    app->main_queue().post(std::bind(&PsychoManage::midi_callback, app, timeStamp, *message));
}

/////////////////////////////////////////////////////////////////

void PsychoManage::setup()
{
    ViewerApp::setup();
...
//    m_light_component->set_lights(lights());
    ...
    m_shaders[SHADER_UNLIT_DISPLACE] = gl::Shader::create(gl::unlit_displace_vert, unlit_frag);
    m_shaders[SHADER_UNLIT_SKIN_DISPLACE] = gl::Shader::create(gl::unlit_skin_displace_vert, unlit_frag);
...
void PsychoManage::update(float timeDelta)
{
...
void PsychoManage::resize(int w, int h)
{
...
void PsychoManage::key_press(const KeyEvent &e)
{
...
void PsychoManage::key_release(const KeyEvent &e)
{
...
void PsychoManage::mouse_press(const MouseEvent &e)
{
...
void PsychoManage::mouse_release(const MouseEvent &e)
{
...
void PsychoManage::mouse_move(const MouseEvent &e)
{
...
void PsychoManage::mouse_drag(const MouseEvent &e)
{
...
void PsychoManage::mouse_wheel(const MouseEvent &e)
{
...
void PsychoManage::file_drop(const MouseEvent &e, const std::vector<std::string> &files)
{
...
void PsychoManage::update_property(const PropertyConstPtr &theProperty)
{
    ...
    } else if(theProperty == m_obj_audio_auto_rotate)
    {
        if(selected_mesh)
        {
            ...
            selected_mesh->set_update_function([this, selected_mesh, v](float td)
                                               {
                                                   float val = td * v;
                                                   selected_mesh->transform() = rotate(selected_mesh->transform(),
                                                                                       glm::radians(val),
...
void PsychoManage::build_skeleton(gl::BonePtr currentBone, vector<vec3> &points,
...
void PsychoManage::setup_offscreen_cameras(int num_screens, bool as_circle)
{
    // remove old objects
...
gl::Texture PsychoManage::create_offscreen_texture()
{
    if(!m_offscreen_fbo)
    {
    ...
    textures()[TEXTURE_OUTPUT] = gl::render_to_texture(m_offscreen_fbo,
                                                       [this, screen_width, screen_height]
                                                       {
                                                           ...
                                                           {
                                                               // set viewport here
                                                               gl::set_window_dimension(
                                                                       vec2(screen_width, screen_height),
                                                                       vec2(i * screen_width, 0));

                                                               if(*m_use_test_pattern)
                                                               {
                                                                   gl::draw_quad(gl::window_dimension(),
                                                                                 m_cam_colors[i]);
                                                               }
...
bool PsychoManage::load_asset(const std::string &the_path, uint32_t the_lvl, bool add_to_scene)
{
    gl::MeshPtr m;
    gl::Texture t;
...
void PsychoManage::update_select_indicator()
{
...
void PsychoManage::reset_lvl(size_t the_lvl)
{
...
void PsychoManage::toggle_selection(int the_inc)
{
    if(m_timer_select_enable.has_expired())
    {
        ...
        if(!m_vec.empty())
        {
            ...
            add_selected_object(next_mesh);
            update_select_indicator();
        }
    }
}

void PsychoManage::process_joystick_input(float time_delta)
{
    gl::MeshPtr m;
    if(!selected_objects().empty())
    {
