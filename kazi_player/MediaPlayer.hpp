...
namespace kinski
{
    ...
    private:
        ...
        m_stream_switch_interval = Property_<float>::create("switch stream every x secs", 75.f);
        
        Property_<uint32_t>::Ptr
        m_broadcast_port = Property_<uint32_t>::create("discovery broadcast port", 55555);
        
        Property_<std::vector<string>>::Ptr
        m_stream_urls = Property_<std::vector<string>>::create("stream urls");
        
        Timer m_stream_timer;
        
        int m_stream_current_index = -1;
