...
void MediaPlayer::setup()
{
    ...
    register_property(m_stream_urls);
    register_property(m_stream_switch_interval);
    ...
    // setup stream switch timer
    m_stream_timer = Timer(main_queue().io_service(), [this]
    {
        if(m_stream_urls->value().empty()){ m_stream_current_index = -1; return; }
        
        m_stream_current_index = (m_stream_current_index + 1) % m_stream_urls->value().size();
        *m_movie_path = m_stream_urls->value()[m_stream_current_index];
    });
    m_stream_timer.set_periodic();
    m_stream_timer.expires_from_now(*m_stream_switch_interval);
}
...
void MediaPlayer::update_property(const Property::ConstPtr &theProperty)
{
    ...
    else if(theProperty == m_stream_switch_interval)
    {
        if(*m_stream_switch_interval > 0.f)
        {
            m_stream_timer.expires_from_now(*m_stream_switch_interval);
        }
        else{ m_stream_timer.cancel(); }
    }
}
