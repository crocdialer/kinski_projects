...
void MediaPlayer::send_sync_cmd()
{
    ...
    for(const auto &ip : m_ip_adresses->value())
    {
        ...
        net::async_send_udp(background_queue().io_service(), cmd, ip, remote_control().udp_port());
...
//    std::unique_lock<std::mutex> lock(g_ip_table_mutex);
    ...
    for(const auto &ip : m_ip_adresses->value())
    {
        net::async_send_tcp(background_queue().io_service(), the_cmd, ip, remote_control().tcp_port());
    }
}
